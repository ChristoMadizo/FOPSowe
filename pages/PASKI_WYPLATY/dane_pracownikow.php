...
$sql = "SELECT id,name,surname,phone_number,e_mail_address FROM `aa01_workers`";
...
if (isset($_POST['action']) && $_POST['action'] === 'sprawdz_odpowiedzi') {  //odświeża potwierdzenie SMS i robi UPDATE bazy, jeśli podano maila
    ...
    foreach ($workers_info as $row) { //iteruje przez wszystkich pracownikow z formularza i jeśli w bazie adres email jest pusty, to szuka SMSa z adresem
        ...
        if (true) {  //BYŁO: empty($adres_email_formularz) jeśli w bazie (czyli w formularzu) adres e-mail jest pusty -
...
<div class="content">
    ...
    <a href="http://192.168.101.203/index.php?page=PASKI_WYPLATY/paski_wyplaty_secure">
        Przejdź do wysyłania pasków
    </a>
                ...
                <td>
                    <form method="post">
                        ...
                        <input type="hidden" name="surname"
                            value="<?php echo htmlspecialchars($row['surname'], ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="phone_number"
                            value="<?php echo htmlspecialchars($row['phone_number'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="e_mail_address"
                            value="<?php echo htmlspecialchars($row['e_mail_address'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                        <input type="hidden" name="text_message_content"
                            value="Proszę o podanie adresu e-mail do wysyłki pasków wynagrodzeń, poprzez odpowiedź na tę wiadomość. Dziękuję, Dział Kadr FOPS.">

                        <!-- Przycisk wysyłki SMS -->
                        <button type="submit" name="action" value="wyslij_sms"
                            style="all: unset; color: blue; text-decoration: underline; cursor: pointer; display: block; text-align: center;">Wyślij
                            SMS</button>
                        <!-- Pole do wpisania treści SMS-a (teraz edytowalne) -->
                <td>
                    <textarea name="text_message_content"
                        style="width: 500px; box-sizing: border-box; word-wrap: break-word; white-space: pre-wrap;"
                        rows="3">Proszę o podanie adresu e-mail do wysyłki pasków wynagrodzeń, poprzez odpowiedź na tę wiadomość. Dziękuję, Dział Kadr FOPS.</textarea>
                </td>
                </form>
                </td>
