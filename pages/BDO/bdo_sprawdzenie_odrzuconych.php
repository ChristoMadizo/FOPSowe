...
$lastRejectedKpoId_new = $filteredItems[1]['kpoId']; // Pobiera KpoId


if (!empty($filteredItems)) {  // Powiadomienie o nowej odrzuconej karcie
    if ($lastRejectedCardNumber_new) {
        $filePath = '/home/kmadzia/www/pages/BDO/ostatnia_odrzucona.txt';
        file_put_contents($filePath, "$lastRejectedCardNumber_new | KpoId: $lastRejectedKpoId_new");

        if ($lastRejectedCardNumber_new !== explode(' ', $lastRejectedCardNumber_old)[0]) {
            foreach (['k.madzia@fops.pl'] as $email) {  // Wysyła maila do dwóch osób
                sendEmail($email,
                    'BDO: nowa odrzucona karta: ' . $lastRejectedCardNumber_new . ' ( ' . $filteredItems[1]["cardRejectionTime"] . ' )',
                    'Nowa odrzucona karta: ' . $lastRejectedCardNumber_new . 
                    "\nKpoId: " . $lastRejectedKpoId_new . 
                    "\nUrl: " . "https://rejestr-bdo.mos.gov.pl/WasteRegister/WasteTransferForwardedCard/EditRejected/" . $lastRejectedKpoId_new,
