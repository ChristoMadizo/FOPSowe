...
$sort_direction = ($table === 'ff_02_cars_serwis_historia') ? 'DESC' : 'ASC';

if ($sort_column) {
    $query = "SELECT $selected_columns FROM `$table` WHERE car_id = $car_id ORDER BY `$sort_column` " . $sort_direction;
    $result = mysqli_query($connection, $query);
    $info_to_display = mysqli_fetch_assoc(mysqli_query($connection, $query));
}
        ...
        columns.forEach(column => {
            if (column !== "id" && column !== "car_id") {
                ...
                if (column === "zadanie_typ") {
                    ...
                    // Dodaj pustą opcję jako domyślną
                ...
                } else if (column === "status_przypomnienia") {
                ...
                } else if (column === "zadanie_deadline") {
                    inputElement = document.createElement("input");
                    inputElement.type = "date";
                    inputElement.name = `dane[${newRowId}][${column}]`;
                    // Set default value to today + 7 days
