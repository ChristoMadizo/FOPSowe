...
$cars_list=fetch_data($connection, "SELECT id FROM ff_01_cars");
...
foreach ($cars_list[1] as $car) {
    ...
    $first_record = $records[1][0];
    $last_record = $records[1][count($records[1]) - 1];

    $start_km = $first_record['stan_licznika'];
    $end_km = $last_record['stan_licznika'];
    $start_date = new DateTime($first_record['serwis_date']);
    $end_date = new DateTime($last_record['serwis_date']);
